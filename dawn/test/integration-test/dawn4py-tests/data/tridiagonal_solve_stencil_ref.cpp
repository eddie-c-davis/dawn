...
namespace cuda{
__global__ void __launch_bounds__(32)  tridiagonal_solve_stencil_stencil49_ms116_kernel(const int isize, const int jsize, const int ksize, const int stride_111_1, const int stride_111_2, ::dawn::float_type * const b, ::dawn::float_type * const c) {
...
c_kcache[0] = (c_kcache[0] / __ldg(&(b[idx111])));
  }
    // Flush of kcaches
...
if(iblock >= 0 && iblock <= block_size_i -1 + 0 && jblock >= 0 && jblock <= block_size_j -1 + 0) {
    ...
    c_kcache[0] =c[idx111+stride_111_2*-1];
...
public:
  ...
  public:
    ...
    void run(storage_ijk_t a_ds, storage_ijk_t b_ds, storage_ijk_t c_ds, storage_ijk_t d_ds) {
      ...
      tridiagonal_solve_stencil_stencil49_ms116_kernel<<<blocks, threads>>>(nx,ny,nz,b_ds.strides()[1],b_ds.strides()[2],(b.data()+b_ds.get_storage_info_ptr()->index(b.begin<0>(), b.begin<1>(),0 )),(c.data()+c_ds.get_storage_info_ptr()->index(c.begin<0>(), c.begin<1>(),0 )));
      };
      {;
      ...
      const unsigned int nbz = 1;
      dim3 blocks(nbx, nby, nbz);
      tridiagonal_solve_stencil_stencil49_ms117_kernel<<<blocks, threads>>>(nx,ny,nz,a_ds.strides()[1],a_ds.strides()[2],(a.data()+a_ds.get_storage_info_ptr()->index(a.begin<0>(), a.begin<1>(),0 )),(b.data()+b_ds.get_storage_info_ptr()->index(b.begin<0>(), b.begin<1>(),0 )),(c.data()+c_ds.get_storage_info_ptr()->index(c.begin<0>(), c.begin<1>(),0 )),(d.data()+d_ds.get_storage_info_ptr()->index(d.begin<0>(), d.begin<1>(),0 )));
      };
      {;
      gridtools::data_view<storage_ijk_t> c= gridtools::make_device_view(c_ds);
      gridtools::data_view<storage_ijk_t> d= gridtools::make_device_view(d_ds);

...
bool checkReadExtents(const std::unique_ptr<iir::Stencil>& stencil, const int maxHaloPoints) {
  ...
  int iMinus = horizExtent.iMinus();
  int iPlus = horizExtent.iPlus();
  int jMinus = horizExtent.jMinus();
  int jPlus = horizExtent.jPlus();
  ...
  if(iPlus > maxHaloPoints || iMinus < -maxHaloPoints || jPlus > maxHaloPoints ||
     jMinus < -maxHaloPoints || kPlus > maxHaloPoints || kMinus < -maxHaloPoints) {
    return false;
  }
...
bool PassStageMerger::run(const std::shared_ptr<iir::StencilInstantiation>& instantiation) {
  ...
  for(const auto& stencil : instantiation->getStencils()) {
    ...
    for(auto& multiStage : stencil->getChildren()) {
      ...
      if(!checkReadExtents(stencil, options.MaxHaloPoints)) {
        return false;
      }
    }
  }
